...
typedef struct {
    ...
    char *handle;
    char *varname;
    Aws::Lambda::LambdaClient *item;
} aws_sdk_tcl_lambda_trace_t;
...
char *aws_sdk_tcl_lambda_VarTraceProc(ClientData clientData, Tcl_Interp *interp, const char *name1, const char *name2, int flags) {
    auto *trace = (aws_sdk_tcl_lambda_trace_t *) clientData;
    if (trace->item == nullptr) {
        ...
        if (!Tcl_InterpDeleted(trace->interp)) {
            Tcl_UntraceVar(trace->interp, trace->varname, TCL_TRACE_WRITES|TCL_TRACE_UNSETS,
                           (Tcl_VarTraceProc*) aws_sdk_tcl_lambda_VarTraceProc,
...
static int aws_sdk_tcl_lambda_CreateCmd(ClientData clientData, Tcl_Interp *interp, int objc, Tcl_Obj *const objv[]) {
    ...
    auto *client = credentials_provider_ptr != nullptr ? new Aws::Lambda::LambdaClient(credentials_provider_ptr, client_config) : new Aws::Lambda::LambdaClient(client_config);
    char handle[80];
...
//                                 (Tcl_CmdDeleteProc*) aws_sdk_tcl_lambda_clientObjCmdDeleteProc);
    ...
    if (objc == 3) {
        auto *trace = (aws_sdk_tcl_lambda_trace_t *) Tcl_Alloc(sizeof(aws_sdk_tcl_lambda_trace_t));
        trace->interp = interp;
...
void aws_sdk_tcl_lambda_InitModule() {
    ...
    if (!aws_sdk_tcl_lambda_ModuleInitialized) {
        ...
        Tcl_CreateExitHandler(aws_sdk_tcl_lambda_ExitHandler, nullptr);
